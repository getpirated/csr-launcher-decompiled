...
function setupGameLaunch() {
  ...
  elements.btnLaunch.addEventListener("click", async () => {
    ...
    if (checkResult.needsUpdate) {
      ...
      if (statusEl)
        statusEl.textContent = tf("modal_downloading", {
          count: checkResult.files.length,
          size: sizeMB
        })
      ...
      const gameDir = elements.csgoDirInput
        ? elements.csgoDirInput.value
        : state.settings.csgoDir
      ...
      if (dlResult.error || dlResult.failed > 0) {
        const err =
          dlResult.error || tf("modal_failed", { failed: dlResult.failed })
        showError(err)
        if (playText) playText.textContent = t("btn_launch")
        if (playIcon) playIcon.className = "fa-solid fa-play"
        elements.btnLaunch.disabled = false
...
async function loadInventory() {
  ...
  try {
    ...
    elements.inventoryCount.textContent = tf("inventory_count", {
      count: items.length
    })
...
function createInventoryItem(item) {
  ...
  const rarityClass =
    rarity < rarityClasses.length && rarityClasses[rarity]
  ...
  const weaponName =
    itemTypeId < weaponNames.length && weaponNames[itemTypeId]
      ? weaponNames[itemTypeId]
      : "Item"
  ...
  const floatText = item.float
    ? tf("float_label", { value: parseFloat(item.float).toFixed(4) })
    : ""
...
function applySettings() {
  if (elements.csgoDirInput)
    elements.csgoDirInput.value = state.settings.csgoDir || ""
  if (elements.launchArgsInput)
    elements.launchArgsInput.value = state.settings.launchArgs || ""
}
