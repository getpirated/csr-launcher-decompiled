...
async function saveAuthCookies() {
  try {
    ...
    const csrCookies = cookies.filter(
      (c) =>
        c.name.includes("jwt") ||
...
function ensureCustomLangFolder() {
  try {
    ...
    console.error(
      "[Lang] Failed to create custom_lang:",
      e.code,
      e.message,
      e.path
    )
...
function startGSIS(port) {
  ...
  gsisServer.on("error", (err) => {
    ...
    if (mainWindow && mainWindow.webContents) {
      mainWindow.webContents.send("gsis-status", {
        running: false,
        error: err.message
      })
    }
    ...
    for (const file of filesToDownload) {
      ...
      try {
        ...
        if (newHash !== file.hash) {
          throw new Error(
            `MD5 mismatch after download: expected ${file.hash}, got ${newHash}`
          )
        }
...
ipcMain.handle("check-csr-updates", async () => {
  try {
    ...
    console.log(
      `[Update] ${filesToDownload.length} files need download (${(totalSize / 1024 / 1024).toFixed(1)} MB)`
    )
            ...
            https
              .get(redirectUrl, (res2) => {
                if (res2.statusCode < 200 || res2.statusCode >= 300) {
                  const err = new Error(
                    `HTTP ${res2.statusCode} on redirect for ${remotePath}`
                  )
